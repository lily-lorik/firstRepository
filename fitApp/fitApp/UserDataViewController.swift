...
class UserDataViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        UserManager.shared.user = User.init(name: nameTextField.text, phoneNumber: phoneNumberTextField.text, currentWeight: UserManager.shared.user?.currentWeight, currentHeight: UserManager.shared.user?.currentHeight, calories: Int(currentCaloriesTextField.text!), isGetWeight: goalSegment.selectedSegmentIndex == 0 ? true : false, period: Int(periodTextLabel.text!))
